...
def test_inequality_cv():
    ...
    x_proj = projection_layer.call(projection_layer.get_init(x), x, n_iter=100)[0]
...
def test_equality_inequality_box_cv(seed, batch_size):
    ...
    # Check that the projection has zero cv
    for ii in range(batch_size):
        projection_layer.eq_constraint.b = b[ii : ii + 1]
        x_proj = projection_layer.call(
            projection_layer.get_init(x), x, b[ii : ii + 1], n_iter=1000
        )[0]
        cv_proj = projection_layer.cv(x_proj[ii : ii + 1])
        assert jnp.allclose(
