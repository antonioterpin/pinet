...
a_aug = jnp.concatenate((a, jnp.eye(m)), axis=1)
# The augmented equality matrix must match the primal-plus-slack dimension.
assert a_aug.shape == (
    m,
    m + n,
), f"Augmented matrix a should have shape ({m}, {m + n}), instead: {a_aug.shape}"

a_aug_inv = jnp.linalg.pinv(a_aug)
...
def project_pinv_vb(xs: jnp.ndarray, b: jnp.ndarray):
    ...
    """
    return xs - a_aug_inv @ (a_aug @ xs - b)
