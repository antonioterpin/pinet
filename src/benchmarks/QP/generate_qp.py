    ...
    if convex:
        ...
        for problem_idx in tqdm(range(n_samples)):
            ...
            constraints = cast(
                list[cp.Constraint],
                [
                    a_mat[0, :, :] @ ycp == x_data[problem_idx, :, 0],
                    g_mat[0, :, :] @ ycp <= h[0, :, 0],
                ],
            )
...
if __name__ == "__main__":
    ...
    a_mat = jax.random.normal(key[2], shape=(1, NUM_EQ, NUM_VAR))
    ...
    h = jnp.expand_dims(
        jnp.sum(jnp.abs(g_mat @ jnp.linalg.pinv(a_mat[0])), axis=1), axis=2
    )
    ...
    if SAVE_DATASET:
        ...
        objectives, y_star = solve_problems(
            q_mat, p[0, :, :], a_mat, x_data, g_mat, h, CONVEX
        )
