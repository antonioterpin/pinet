...
def test_instantiation_error():
    try:
        ...
        raise AssertionError(
            "No check that a_mat is a matrix with shape "
    ...
    try:
        ...
        raise AssertionError("Number of rows in a_mat must equal size of b.")
...
def test_equality_eye(method, seed, n_batch_a, n_batch_b, n_batch_x):
    # a_mat and b always have smaller or equal batch size than that of x.
    ...
    # Equality constraint
    a_mat = jnp.repeat(jnp.expand_dims(jnp.eye(DIM), axis=0), n_batch_a, axis=0)
    ...
    eq_constraint = EqualityConstraint(a_mat, b, method=method)
    # Prepare input
...
def test_equality_generic_invertible(method, seed, n_batch_a, n_batch_b, n_batch_x):
    ...
    a_mat = jax.random.uniform(key[0], shape=(n_batch_a, DIM, DIM))
    # Normalize
    a_mat = (a_mat @ jnp.matrix_transpose(a_mat) + jnp.eye(DIM)) + (
        a_mat - jnp.matrix_transpose(a_mat)
    )
    b = jax.random.uniform(key[1], shape=(n_batch_b, DIM, 1))
    x = jax.random.uniform(key[2], shape=(n_batch_x, DIM, 1))
    # True projection is a_mat^{-1} @ b (a_mat is invertible)
    y = jnp.linalg.solve(a_mat, b)
...
def test_equality_qp(method, seed, n_batch_a, n_batch_b, n_batch_x):
    ...
    key = jax.random.PRNGKey(seed)
    key = jax.random.split(key, num=3)
    a_mat = jax.random.uniform(key[0], shape=(n_batch_a, N_EQ, DIM))
    # Generate RHS b vector
    if n_batch_a > n_batch_b:
        # If we have more a_mat than b, then we need to find
        ...
        for ii in range(n_batch_a):
            ...
            constraints += [a_mat[ii, :, :] @ x_list[ii] == b_common]
        ...
        # If we have more b than a_mat, then we generate b in the range of a_mat
        b = a_mat @ jax.random.uniform(key[1], shape=(n_batch_b, DIM, 1))
...
def test_equality_method_none_raises_not_implemented():
    ...
    a_mat = jnp.expand_dims(jnp.eye(1), axis=0)  # (1, 1, 1)
    b = jnp.zeros((1, 1, 1))
    eq_constraint = EqualityConstraint(a_mat, b, method=None)
...
def test_equality_invalid_method_raises_exception():
    ...
    with pytest.raises(
        ...
        EqualityConstraint(a_mat, b, method="foo")
