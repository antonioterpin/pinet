...
def test_project_eq_ineq_var_a_dyn_varb(seed, batch_size):
    ...
    eq_constraint = EqualityConstraint(a=a_dyn_new, b=b_new, method=method, var_a=True)
    projection_layer = Project(eq_constraint=eq_constraint)
    inp = ProjectionInstance(
        x=xinfeas[..., None],
        eq=EqualityConstraintsSpecification(a=a_dyn_new, b=b_new),
    )
    ...
    inp = ProjectionInstance(
        x=xinfeas[..., None],
        eq=EqualityConstraintsSpecification(b=b_new, a=a_dyn_new),
    )
...
def test_call_and_check_invalid_reduction_raises(bad_reduction):
    ...
    a = jax.random.normal(k_a_dyn, (batch, n_eq, dim))
    x0 = jax.random.normal(kx0, (batch, dim, 1))
    b = a @ x0
